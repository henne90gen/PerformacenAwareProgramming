...
BM_ParsePointsGeneric(benchmark::State &state) {
    ...
    auto s = ss.str();
    for (auto _: state) {
        auto pointPairs = ParsePointPairsGeneric({ s.data(), s.size() });
        benchmark::DoNotOptimize(pointPairs);
    }
}
